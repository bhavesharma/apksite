...
for  apps in $(cat /tmp/appid.txt)
  do
          ...
          mysql --host=$_hostsrv --user=$_db_user --password=$_db_password $_db -e "select CONCAT_WS('\n',icon,version,size,updated,md5,androidVersionText,scoreText,screenshots,video,title,descriptionHTML) from apkdetails where appid ='$apps'  INTO OUTFILE '/data/$1/$apps.txt';"
